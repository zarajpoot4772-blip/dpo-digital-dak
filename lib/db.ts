...
async function init(){
 ...
 CREATE TABLE IF NOT EXISTS password_history(id SERIAL PRIMARY KEY,user_id INTEGER NOT NULL REFERENCES users(id),password_hash TEXT NOT NULL,changed_by INTEGER REFERENCES users(id),changed_at TIMESTAMPTZ NOT NULL DEFAULT now());
 ...
 CREATE INDEX IF NOT EXISTS idx_daks_status ON daks(status); CREATE INDEX IF NOT EXISTS idx_daks_assigned ON daks(assigned_to); CREATE INDEX IF NOT EXISTS idx_actions_dak ON actions(dak_id,created_at); CREATE INDEX IF NOT EXISTS idx_file_notes_dak ON file_notes(dak_id,created_at); CREATE INDEX IF NOT EXISTS idx_notifications_user ON notifications(user_id,is_read); CREATE INDEX IF NOT EXISTS idx_password_history_user ON password_history(user_id,changed_at DESC); CREATE INDEX IF NOT EXISTS idx_login_throttle_updated ON login_throttle(updated_at);
 `);
 // Idempotent local-schema migration for workspaces created before Branch Head support.
 await db.exec(`ALTER TABLE daks ADD COLUMN IF NOT EXISTS due_date DATE; CREATE TABLE IF NOT EXISTS file_notes(id SERIAL PRIMARY KEY,dak_id INTEGER NOT NULL REFERENCES daks(id),user_id INTEGER NOT NULL REFERENCES users(id),body TEXT NOT NULL,created_at TIMESTAMPTZ NOT NULL DEFAULT now()); CREATE INDEX IF NOT EXISTS idx_file_notes_dak ON file_notes(dak_id,created_at); ALTER TABLE users ADD COLUMN IF NOT EXISTS must_change_password BOOLEAN NOT NULL DEFAULT false; ALTER TABLE users ADD COLUMN IF NOT EXISTS password_changed_at TIMESTAMPTZ; UPDATE users SET password_changed_at=COALESCE(password_changed_at,created_at,now()) WHERE password_changed_at IS NULL; ALTER TABLE users ALTER COLUMN password_changed_at SET DEFAULT now(); ALTER TABLE users ALTER COLUMN password_changed_at SET NOT NULL; CREATE TABLE IF NOT EXISTS password_history(id SERIAL PRIMARY KEY,user_id INTEGER NOT NULL REFERENCES users(id),password_hash TEXT NOT NULL,changed_by INTEGER REFERENCES users(id),changed_at TIMESTAMPTZ NOT NULL DEFAULT now()); CREATE INDEX IF NOT EXISTS idx_password_history_user ON password_history(user_id,changed_at DESC); CREATE TABLE IF NOT EXISTS login_throttle(lock_key TEXT PRIMARY KEY,username TEXT NOT NULL,ip TEXT NOT NULL,failed_count INTEGER NOT NULL DEFAULT 0,locked_until TIMESTAMPTZ,updated_at TIMESTAMPTZ NOT NULL DEFAULT now()); CREATE INDEX IF NOT EXISTS idx_login_throttle_updated ON login_throttle(updated_at); ALTER TABLE users ADD COLUMN IF NOT EXISTS totp_secret TEXT; ALTER TABLE users ADD COLUMN IF NOT EXISTS totp_pending_secret TEXT; ALTER TABLE users ADD COLUMN IF NOT EXISTS totp_enabled BOOLEAN NOT NULL DEFAULT false; CREATE TABLE IF NOT EXISTS mfa_challenges(id TEXT PRIMARY KEY,user_id INTEGER NOT NULL REFERENCES users(id),expires_at TIMESTAMPTZ NOT NULL,created_at TIMESTAMPTZ NOT NULL DEFAULT now()); CREATE INDEX IF NOT EXISTS idx_mfa_challenges_expiry ON mfa_challenges(expires_at); ALTER TABLE users DROP CONSTRAINT IF EXISTS users_role_check; ALTER TABLE users ADD CONSTRAINT users_role_check CHECK(role IN ('ADMIN','DPO','CLERK','OFFICER','BRANCH_HEAD')); ALTER TABLE user_signatures ADD COLUMN IF NOT EXISTS asset_type TEXT NOT NULL DEFAULT 'SIGNATURE'; ALTER TABLE documents ADD COLUMN IF NOT EXISTS search_text TEXT; ALTER TABLE documents ADD COLUMN IF NOT EXISTS text_indexed BOOLEAN NOT NULL DEFAULT false; ALTER TABLE documents DROP CONSTRAINT IF EXISTS documents_version_type_check; ALTER TABLE documents ADD CONSTRAINT documents_version_type_check CHECK(version_type IN ('ORIGINAL','CONVERTED','SUPPORTING','SUPPORTING_CONVERTED','APPROVED','REJECTED_COPY')); DROP INDEX IF EXISTS idx_user_signatures_active; CREATE UNIQUE INDEX IF NOT EXISTS idx_user_signature_asset_active ON user_signatures(user_id,asset_type) WHERE active=true;`);
