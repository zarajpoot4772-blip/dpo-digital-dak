...
async function init(){
 ...
 CREATE TABLE IF NOT EXISTS documents(id SERIAL PRIMARY KEY,dak_id INTEGER NOT NULL REFERENCES daks(id),version_type TEXT NOT NULL CHECK(version_type IN ('ORIGINAL','CONVERTED','SUPPORTING','SUPPORTING_CONVERTED','APPROVED','REJECTED_COPY')),original_filename TEXT NOT NULL,stored_filename TEXT UNIQUE NOT NULL,file_type TEXT NOT NULL,file_size INTEGER NOT NULL,sha256 TEXT NOT NULL,search_text TEXT,uploaded_by INTEGER NOT NULL REFERENCES users(id),uploaded_at TIMESTAMPTZ NOT NULL DEFAULT now(),immutable BOOLEAN NOT NULL DEFAULT true);
